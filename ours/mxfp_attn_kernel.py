...
from ours.quant_kernels import quant_fpxint8, quant_mxfp8e5, quant_mxfp4, quant_nvfp4, get_nvfp4_scale, quant_mxfp8e5_diag
import os
...
def mxfp_attn_kernel(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, smooth_k=False, attention_sink=False, tensor_layout="HND",
                    ...
                    save_qk=False, quant_granularity="tokenwise", fuse_mp_quant=True, oneline_quant=False):

    torch.cuda.set_device(v.device)
    if 'diag' in block_scale_type and oneline_quant:
        assert fuse_mp_quant == False, "fuse_mp_quant must be False when using oneline_quant"
    ...
    q_scale_nvfp4, k_scale_nvfp4, q_fp4, k_fp4 = None, None, None, None
    ...
    if block_scale_type == "mxfp4":
        ...
        q_fp8, q_scale, k_fp8, k_scale, q_fp4, q_nvfp4_scale, k_fp4, k_nvfp4_scale, q_scale_2, k_scale_2 = \
            quant_mxfp8e5_diag(q, k, BLKQ=BLKQ, BLKK=BLKK, pack_along_lastdim=pack_along_lastdim)
        q_quant = q_fp8
        k_quant = k_fp8
        ...
        if 'diag' in block_scale_type:
            pack_along_lastdim = True
            # q_nvfp4_scale, k_nvfp4_scale = get_nvfp4_scale(q, k, BLKQ=BLKQ, BLKK=BLKK)
            q_fp4, q_nvfp4_scale, k_fp4, k_nvfp4_scale, q_scale_2, k_scale_2 = quant_nvfp4(q, k, BLKQ=BLKQ, BLKK=BLKK, \
                pack_along_lastdim=pack_along_lastdim, dual_scale=dual_scale, quant_granularity=quant_granularity)
    
    elif "mxfp8" in block_scale_type and oneline_quant:
    ...
    if M_padded > M:
        ...
        if 'diag' in block_scale_type:
            q_scale_nvfp4_padded = torch.zeros(B, H, M_padded, K //16, device=q_nvfp4_scale.device, dtype=q_nvfp4_scale.dtype)
            q_scale_nvfp4_padded[:, :, :M, :] = q_nvfp4_scale
            q_scale_nvfp4 = q_scale_nvfp4_padded
    ...
    if N_padded > N:
        ...
        if 'diag' in block_scale_type:
            k_scale_nvfp4_padded = torch.zeros(B, H, N_padded, K//16, device=k_nvfp4_scale.device, dtype=k_nvfp4_scale.dtype)
            k_scale_nvfp4_padded[:, :, :N, :] = k_nvfp4_scale
            k_scale_nvfp4 = k_scale_nvfp4_padded
    ...
    if 'diag' in block_scale_type:
        q_scale_nvfp4 = q_scale_nvfp4.reshape(B, H, M_padded//128, 4, 32, K //16//4, 4).permute(0, 1, 2, 5, 4, 3, 6).contiguous()
        k_scale_nvfp4 = k_scale_nvfp4.reshape(B, H, N_padded//128, 4, 32, K //16//4, 4).permute(0, 1, 2, 5, 4, 3, 6).contiguous()
    ...
    output = block_scaled_batched_attn(
            ...
            kernel_name=kernel_name,  # mxfp8_
    )   
    if return_quant_tensor:
